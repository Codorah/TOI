...
(() => {
  ...
  function setupCanvas() {
    ...
    el.canvas.width = normalized;
    el.canvas.height = normalized;
  ...
  function shufflePieces() {
    state.pieces.forEach((p) => {
      p.currentX = Math.random() * (el.canvas.width - state.pieceSize);
      p.currentY = Math.random() * (el.canvas.height - state.pieceSize);
      p.rotation = Math.floor(Math.random() * 4) * ROTATE_STEP;
      p.solved = false;
    });
  }

  function buildJigsawPath(piece) {
    ...
    if (edges.t === 0) {
      ...
      ctx.lineTo(-neck, top);
      ctx.bezierCurveTo(-neck * 0.45, top, -neck * 0.75, top + bulge, 0, top + bulge);
      ctx.bezierCurveTo(neck * 0.75, top + bulge, neck * 0.45, top, neck, top);
      ctx.lineTo(right, top);
    }
    ...
    if (edges.r === 0) {
      ...
      ctx.bezierCurveTo(right, -neck * 0.45, right + bulge, -neck * 0.75, right + bulge, 0);
      ctx.bezierCurveTo(right + bulge, neck * 0.75, right, neck * 0.45, right, neck);
      ctx.lineTo(right, bottom);
    }
    ...
    if (edges.b === 0) {
      ...
      ctx.bezierCurveTo(neck * 0.45, bottom, neck * 0.75, bottom + bulge, 0, bottom + bulge);
      ctx.bezierCurveTo(-neck * 0.75, bottom + bulge, -neck * 0.45, bottom, -neck, bottom);
      ctx.lineTo(left, bottom);
    }
    ...
    if (edges.l === 0) {
      ...
      ctx.bezierCurveTo(left, neck * 0.45, left + bulge, neck * 0.75, left + bulge, 0);
      ctx.bezierCurveTo(left + bulge, -neck * 0.75, left, -neck * 0.45, left, -neck);
      ctx.lineTo(left, top);
    }
  ...
  function drawPuzzle() {
    ...
    state.pieces.forEach((p) => {
      const cx = p.currentX + state.pieceSize / 2;
      const cy = p.currentY + state.pieceSize / 2;
      const edges = p.edges || { t: 0, r: 0, b: 0, l: 0 };
      ...
      const sx = Math.max(0, p.sx - srcExtLeft);
      const sy = Math.max(0, p.sy - srcExtTop);
      const sw = Math.max(1, Math.min(state.img.width, p.sx + state.srcW + srcExtRight) - sx);
      const sh = Math.max(1, Math.min(state.img.height, p.sy + state.srcH + srcExtBottom) - sy);
      ...
      ctx.shadowBlur = p === state.selected ? 16 : (p.solved ? 5 : 10);
      ctx.shadowOffsetY = p.solved ? 1 : 3;
      ctx.drawImage(
        state.img,
        sx,
        sy,
        sw,
        sh,
        -state.pieceSize / 2 - destExtLeft,
        -state.pieceSize / 2 - destExtTop,
        state.pieceSize + destExtLeft + destExtRight,
        state.pieceSize + destExtTop + destExtBottom
      );
      ...
      buildJigsawPath(p);
      if (p === state.selected) {
        ctx.strokeStyle = "rgba(255,79,93,0.95)";
        ctx.lineWidth = 2.1;
        ctx.stroke();
      } else if (!p.solved) {
  ...
  function onPointerMove(e) {
    ...
    const { x, y } = toCanvas(e);
    const pad = state.pieceSize * JIGSAW_HIT_PADDING_RATIO;
    state.selected.currentX = clamp(x - state.offsetX, -pad, el.canvas.width - state.pieceSize + pad);
    state.selected.currentY = clamp(y - state.offsetY, -pad, el.canvas.height - state.pieceSize + pad);
  ...
  function onResize() {
    ...
    state.pieces.forEach((p) => {
      ...
      if (p.solved) {
        ...
        p.currentX = clamp(p.currentX, -pad, el.canvas.width - state.pieceSize + pad);
        p.currentY = clamp(p.currentY, -pad, el.canvas.height - state.pieceSize + pad);
      }
    });
  }
  ...
  function bindEvents() {
    ...
    el.canvas.addEventListener("lostpointercapture", () => { state.selected = null; });
